...
def get_match_timeline(match_id):
    """
    This function asks Sportradar for the timeline (events) of a specific match.
    """
    ...
    # If we hit the rate limit (Status 429), pause for 5 seconds and try again
    elif response.status_code == 429:
        print(f"   ⚠️ Rate limit hit on {match_id}. Sleeping for 5 seconds...")
        time.sleep(5)
        return get_match_timeline(match_id)
    # If something else goes wrong, skip this match
    else:
...
if __name__ == "__main__":
    ...
    for i, match_id in enumerate(MATCH_IDS):
        ...
        # THE COOLDOWN: Sleep for 1.5 seconds to avoid getting banned
        time.sleep(1.5)
